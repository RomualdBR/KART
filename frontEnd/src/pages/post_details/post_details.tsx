...
export default function PostDetails() {
    ...
    async function getComments() {
        "use server";
        const comments: Comment[] = await fetch(`http://localhost:3000/comment?post_id=${id}`, {
            method: "GET",
            headers: {
                "Content-Type": "application/json"
            }
                    ...
                    <div className="p-4">
                        <form action={createComment} className="flex-1">
                            <div className="flex items-center gap-2">
                                <input type="text" name="comment_content" className="w-full border-2 border-gray-200 rounded-lg p-4 text-gray-800 resize-none transition-colors" placeholder="Ajouter un commentaire" />
                                <button type="submit" className="bg-blue text-white font-medium rounded-lg">Ajouter</button>
                            </div>
                        </form>
                    </div>
                </div>
